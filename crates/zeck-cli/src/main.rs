...
use argos_core::{
    zcashd_phrase::ZcashdPhraseKeySource, GapLimits, ZcashdSeedKind, PHRASE_CANNOT_RECOVER,
};
use argos_core::{
...
struct Cli {
    ...
    /// Treat the phrase as a zcashd *emergency recovery phrase* rather than
    /// a ZecWallet Lite seed, and search zcashd's legacy derivation paths.
    ///
    /// This is the route for a user who kept the phrase zcashd 4.7.0 showed
    /// them but no longer has `wallet.dat`. The two phrase types look
    /// identical — both are 24 BIP-39 words — but they derive completely
    /// different addresses, so this cannot be auto-detected and must be
    /// asked for.
    ///
    /// The phrase itself is read exactly as `--seed-file` is: from that
    /// file if given, otherwise prompted for. There is deliberately no flag
    /// carrying the phrase, so it cannot land in shell history or `ps`.
    #[arg(long, conflicts_with_all = ["wallet_file", "sapling_key_file", "sprout_key_file"])]
    zcashd_phrase: bool,

    /// Which zcashd era to derive for. Only meaningful with
    /// `--zcashd-phrase`.
    ///
    /// `auto` searches both and is the default, because a user rarely knows
    /// which zcashd version created the wallet and guessing wrong produces
    /// an empty scan rather than an error.
    #[arg(long, value_enum, default_value_t = ZcashdEraArg::Auto, requires = "zcashd_phrase")]
    zcashd_era: ZcashdEraArg,
...
/// Which zcashd derivation era to search.
///
/// The distinction is not cosmetic. A post-4.7 wallet's phrase is an
/// ordinary BIP-39 mnemonic whose PBKDF2 seed drives derivation; a wallet
/// upgraded *into* 4.7 had its pre-existing random seed re-encoded as the
/// phrase's raw entropy, and that entropy is the seed. The same words
/// therefore yield two unrelated key sets.
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
enum ZcashdEraArg {
    /// Search both eras. The default: guessing wrong yields an empty scan
    /// rather than an error, so searching both is the safe choice.
    Auto,
    /// zcashd 4.7.0 or later only — legacy Sapling at
    /// `m/32'/coin_type'/0x7FFFFFFF'/i'` plus legacy transparent.
    Post47,
    /// Wallets upgraded from before 4.7.0 only — Sapling at
    /// `m/32'/coin_type'/account'` over the phrase's raw entropy.
    Pre47,
}
...
async fn main() -> Result<()> {
    ...
    let (key_source, seed_phrase, imported): (
        ...
        (None, None) if cli.zcashd_phrase => {
            // Bail before prompting, as the wallet-file arm does.
            if matches!(cli.command, Commands::InspectWallet) {
                bail!("inspect-wallet needs --wallet-file or --sapling-key-file");
            }

            let phrase = load_phrase(
                cli.seed_file.clone(),
                "Enter your zcashd emergency recovery phrase",
            )?;

            let gaps = GapLimits::uniform(cli.gap_limit);
            let source = Arc::new(match cli.zcashd_era {
                ZcashdEraArg::Auto => {
                    ZcashdPhraseKeySource::derive_all(phrase, network, gaps)?
                }
                ZcashdEraArg::Post47 => ZcashdPhraseKeySource::new(
                    phrase,
                    network,
                    ZcashdSeedKind::Bip39,
                    gaps,
                )?,
                ZcashdEraArg::Pre47 => ZcashdPhraseKeySource::new(
                    phrase,
                    network,
            ...
            eprintln!("Derived {}.", source.describe());
            eprintln!();
            eprintln!("{PHRASE_CANNOT_RECOVER}");
            eprintln!();

            // `seed_phrase` stays `None` deliberately, which disables
            // `show-keys` and birthday auto-detection for this route. Both
            // derive *standard* ZIP-32 accounts from the phrase, and those
            // are not the addresses a zcashd wallet used — they would show
            // the user addresses that were never theirs and probe the wrong
            // ones for history. Pass `--birthday` instead.
            (source, None, None)
        }
        (None, None) => {
    ...
    match cli.command {
        ...
        Commands::ShowKeys if cli.zcashd_phrase => {
            // A zcashd phrase has no standard HD accounts to show — its
            // addresses are on the legacy paths — so list those instead of
            // refusing. This is the only offline check a phrase-only
            // recovery has: with no wallet file to compare against, the
            // addresses themselves are the evidence the right derivation
            // ran. A user can check one against an old block explorer
            // bookmark before spending hours on a scan.
            let keys = key_source
                .imported_keys()
                .expect("a zcashd phrase source always carries derived keys");

            let sapling = argos_core::zcashd_phrase::sapling_addresses(keys, network)?;
            let transparent = imported_transparent_keys(keys)?;

            println!("━━━ Legacy Sapling ({}) ━━━", sapling.len());
            for (index, address) in sapling.iter().enumerate() {
                println!("  [{index:>3}] {address}");
            }

            println!();
            println!("━━━ Legacy transparent ({}) ━━━", transparent.len());
            for (index, key) in transparent.iter().enumerate() {
                println!(
                    "  [{index:>3}] {}",
            ...
            println!();
            println!(
                "These are the addresses a scan will search. Widen with --gap-limit \
                 if the address you are looking for is not listed."
            );
            return Ok(());
        }
        Commands::ShowKeys => {
...
fn load_seed_phrase(seed_file: Option<PathBuf>) -> Result<SecretString> {
    load_phrase(seed_file, "Enter your 24-word seed phrase")
}

/// Read a recovery phrase from a permission-checked file, or prompt for it.
///
/// The prompt is a parameter because a zcashd user is not holding what they
/// would call a "seed phrase" — they are holding the emergency recovery
/// phrase zcashd 4.7.0 displayed once. Asking for the wrong noun invites
/// them to conclude they have the wrong thing and stop.
fn load_phrase(seed_file: Option<PathBuf>, prompt: &str) -> Result<SecretString> {
