...
///
/// Two phrasings, because `zcash_client_backend` has two. The shielding and
/// change-generation paths say "Insufficient funds: required N, but only M
/// were available"; `propose_send_max_transfer` — the one `imported_sweep`
/// calls — says "Insufficient balance (have 0, need 10001 including fee)".
/// Matching only the first meant a multi-account imported sweep aborted on
/// the first empty account instead of skipping it, so a wallet whose funded
/// Sapling key sat behind an empty one swept nothing and reported a hard
/// failure. Found by driving a phrase-only recovery against regtest, where
/// account 0 is empty and account 2 holds the funds.
...
mod tests {
    ...
    fn insufficient_funds_proposal_errors_are_detected_for_skip() {
        ...
        // `propose_send_max_transfer`'s phrasing, which `imported_sweep`
        // hits. Verbatim from a regtest run: before this was matched, a
        // multi-account imported sweep aborted on the first empty account
        // and never reached the funded one.
        assert!(is_insufficient_funds_error(
            "transaction build failed: proposing the Sapling sweep: \
